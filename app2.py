...
@st.cache_resource
def load_artifacts():
    svm_model = joblib.load('svm_ticket_model.pkl')
    priority_model = joblib.load('priority_model.pkl')
    tf_idf = joblib.load('tfidf_vectorizer.pkl')
    X_train_tf_idf = joblib.load('X_train_tf_idf.pkl')
    X_train = joblib.load('X_train.pkl')
    y_train = joblib.load('y_train.pkl')
    return svm_model, priority_model, tf_idf, X_train_tf_idf, X_train, y_train

svm_model, priority_model, tf_idf, X_train_tf_idf, X_train, y_train = load_artifacts()
...
if st.button("Analyze Ticket"):
    if user_input.strip():
        ...
        pred_category = svm_model.predict(vec_input)[0]
        pred_priority = priority_model.predict(vec_input)[0]
